...
char	*f_next_data_num_pos(char *tmp, t_ft_print_info *info)
{
	...
	if ((info->d_wide >= 0) && ((size_t)info->d_wide >= ft_strlen(tmp) + 1))
	{
		...
		if (ft_cins('-', info->d_flag))
			info->o_str = f_next_data_num_pos_dash(tmp, char_str_len, info);
		else if (ft_cins('0', info->d_flag) && (info->d_prec < 0))
			info->o_str = f_next_data_num_pos_zero(tmp, char_str_len, info);
		else
			info->o_str = f_next_data_num_pos_space(tmp, char_str_len, info);
		info->o_len = info->d_wide;
	}
	else
	{
		info->o_str = f_next_data_num_pos_else(tmp, info);
